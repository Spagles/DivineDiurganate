...
namespace DivineDiurganate
{
    ...
    public class WorldComp_FaithSystem : WorldComponent
    {
        ...
        private Dictionary<Pawn, float> leaderFaithCache = new Dictionary<Pawn, float>();
        ...
        public bool CheckForLeaderChange()
        {
            ...
            if (newLeader != currentLeader)
            {
                ...
                // 如果是新领袖，保存旧领袖的信仰值（如果有）
                if (oldLeader != null && leaderFaithCache.ContainsKey(oldLeader))
                {
                    leaderFaithCache[oldLeader] = currentFaith;
                }

                // 如果是已有领袖，恢复其信仰值
                if (newLeader != null && leaderFaithCache.ContainsKey(newLeader))
                {
